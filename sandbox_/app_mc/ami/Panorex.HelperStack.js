...
var HelpersBorder        = AMI.BorderHelper;
var HelpersBoundingBox   = require("./Helpers.BoundingBox.js").HelpersBoundingBox;
var HelpersSlice         = AMI.SliceHelper;
var HelpersPanoramic     = require("./Panorex.HelperPanoramic.js").HelpersPanoramic;

class HelpersStack extends THREE.Object3D {
  constructor(stack,geom,options) {  //DEV
    //
    super();
  ...
  set geom(new_geometry) {
    // DEV
    this._geometry = new_geometry;

    this._slice._update(new_geometry);

    //update bb
    this._bBox._update(this._options.dimIJK, this._options.halfDimIJK, this._stack.ijk2LPS);
  ...
  set orientation(orientation) {
    ...
    // also update the border
    this._border.helpersSlice = this._slice; // DEV this draws geometry lines over panorex
  }
  ...
  _create() {
    if (this._stack) {
      ...
      this._prepareBBox();
      this._prepareSlice();
      this._prepareBorder(); // DEV this eliminates lines over panorex
      // todo: Arrow
    } else {
  ...
  _prepareBBox() {
    ...
    this._bBox = new HelpersBoundingBox(this._options.dimIJK, this._options.halfDimIJK, this._stack.ijk2LPS);
    // this._bBox = new HelpersBoundingBox(this._stack); //DEV
    this.add(this._bBox);
  }
  ...
  _prepareSlice() {
    ...
    // DEV
    this._slice = new HelpersPanoramic(this._stack, this._geometry, this._options, this._index, position, direction);
  ...
  updateOptions(options) {
    this._options = options;
    this._slice.updateShaderProps(options);
  }
}
