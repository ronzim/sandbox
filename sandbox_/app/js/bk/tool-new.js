...
var findIntersection = function(dataStruct, plane){
  ...
  do {
    ...
    } while (oppositeSide > 0 && cellScan < 3)

    offset++;

  } while (oppositeSide > 0 && offset <= howManyCells)
  ...
  console.log("looking for first intersection: " + (Date.now()-time2) +" ms");

  // DRAW NEAREST AND INTERSECTING POINTS
  var dotsGeometry = new THREE.Geometry();
  var dotsMaterial = new THREE.PointsMaterial( { size: 10, sizeAttenuation: false } );
  var dots = new THREE.Points( dotsGeometry, dotsMaterial );
  dotsGeometry.vertices.push(new THREE.Vector3(filteredVertices[startingPoint*3],filteredVertices[startingPoint*3+1],filteredVertices[startingPoint*3+2]))
