...
var createDataStruct = function(bufferArray){
  ...
  const endOfArray = -99999;

  var verticesArray = new Float32Array(bufferArray);
  var filteredVertices = new Float32Array(verticesArray.length/3);
  filteredVertices.fill(endOfArray);
  var filteredVerticesIndex = 0;
  ...
  for (var i=0; i<verticesArray.length; i+=3){
    ...
    if (matches.length == 0){
      filteredVertices[filteredVerticesIndex] = currentVertex[0];
      filteredVertices[filteredVerticesIndex+1] = currentVertex[1];
      filteredVertices[filteredVerticesIndex+2] = currentVertex[2];
      filteredVerticesIndex += 3;
    }
  }

  // cutting off exeeding values

  var cutOff = filteredVertices.indexOf(endOfArray);
  newFilteredVertices = filteredVertices.slice(0,cutOff);
