...
var createDataStruct = function(bufferArray){
  ...
  var cellsArray = new Uint32Array(verticesArray.length/3 + verticesArray.length/9); //n vertici + id cella
  var cellId=0;
  ...
  for (var j=0; j<verticesArray.length; j+=3){
    ...
    if ((j/3)%3 == 0){
      cellId += 1;
      cellsArray[cellsArrayIndex] = cellId;
      cellsArray[cellsArrayIndex+1] = vertexId;
      cellsArrayIndex += 2;
    }
    else{
      cellsArray[cellsArrayIndex] = vertexId;
      cellsArrayIndex += 1;
    }
  ...
  var linksArray = new Int32Array(filteredVertices.length*10); // hp 10 link/node
  linksArray.fill(-1);
  ...
  for (var idVertex=0; idVertex<filteredVertices.length; idVertex+=3){
    ...
    linksArray[linksArrayIndex] = idVertex;
    idPositionArray[idVertex/3] = linksArrayIndex;
    linksArrayIndex += 1;
    ...
    for (var q=0; q<cellsArray.length; q++){
      ...
      if (idVertex == cellsArray[q]){
        ...
        linksArray[linksArrayIndex] = currentCid;
        linksArrayIndex += 1;
  ...
  var cutOff = linksArray.indexOf(-1);
  var newlinksArray = linksArray.slice(0,cutOff-1);
  var linksArray = new Int32Array(newlinksArray);
